...
class Computer(private val program: List<String>) {
    data class ExecutionResult(val executedCommand: String?, val jumped: Boolean, val terminated: Boolean)
    ...
    fun executeNextInstruction(): ExecutionResult {
        if (ip !in program.indices) {
            return ExecutionResult(executedCommand = null, jumped = false, terminated = true)
        }
    ...
    private fun executeInstruction(instruction: String): ExecutionResult {
        ...
        return ExecutionResult(parts[0], jumped, terminated = false)
    }
